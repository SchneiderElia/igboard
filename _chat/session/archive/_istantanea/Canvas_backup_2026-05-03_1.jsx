...
export default function Canvas({ design }) {
  ...
  const [feedbackStatus, setFeedbackStatus] = useState(null); // null, 'sending', 'saved'

  useEffect(() => {
    // Resetta lo status del feedback se cambia il design
  ...
  const handleFeedback = async (rating) => {
    if (!design || !design.isDynamic) return;
    setFeedbackStatus('sending');
    try {
      const res = await fetch('/api/feedback-grid', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ design, rating })
      });
      if (res.ok) setFeedbackStatus('saved');
